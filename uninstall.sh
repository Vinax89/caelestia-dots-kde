...
restore_or_remove() {
    ...
    local old_target="$parent/.caelestia-restore-old.$$.${RANDOM}"
    if [[ -e "$target" || -L "$target" ]] && ! mv -- "$target" "$old_target"; then
        rm -rf -- "$staging"
        warn "Failed to stage existing $target - leaving it untouched"
        return 1
    fi
    ...
    rm -rf -- "$staging"
    warn "Failed to install restored $name - original target was restored when possible"
    return 1
}
