...
Singleton {
    ...
    // Kept (CLI-based) on top of the NmQt backend: WirelessPassword tracks the
    // UUID of the connection it just created so a failed attempt deletes only
    // that profile instead of forgetting a previously saved one by SSID.
