...
static bool isSafeUrl(const QUrl& url) {
    // Requests is reachable from all QML; anything it fetches must be a
...
static void applyHeaders(QNetworkRequest& request, const QJSValue& headers) {
    ...
    // Callers may only override a small set of benign headers; anything else
    // (Host, Authorization, Cookie, ...) stays under this class's control, and
    // embedded newlines are rejected outright to prevent header injection.
    ...
    while (it.hasNext()) {
        ...
        if (!allowedHeaders.contains(name) || value.contains('\r') || value.contains('\n')) {
            qCWarning(lcRequests) << "refusing unsafe HTTP header" << name;
            continue;
        }
...
int Requests::registerReply(QNetworkReply* reply, QJSValue callback, QJSValue onError, int timeoutMs) {
    ...
    // Cap in-memory response bodies (get/post; downloads stream to disk with
    // their own progress/size handling) so a hostile endpoint cannot exhaust
    // RAM. The read buffer gets headroom above the limit on purpose:
    // QNetworkReply throttles itself to the read-buffer size, so a buffer set
    // exactly to the limit keeps bytesAvailable() at or below it and the
    // comparison below would never fire.
    ...
    QObject::connect(reply, &QNetworkReply::readyRead, reply, [reply]() {
        if (reply->bytesAvailable() > maxResponseBytes) {
            qCWarning(lcRequests) << "aborting response larger than" << maxResponseBytes << "bytes";
            reply->abort();
        }
    });
